...
const parkImages: Record<string, string> = {
  "sc-2d-mikumi": "/packages images/mikumi.jpg",
  "sc-3d-mikumi": "/packages images/mikumi.jpg",
  "sc-2d-nyerere": "/packages images/nyerere-elephants.jpg",
  "sc-3d-nyerere": "/packages images/nyerere-elephants.jpg",
  "sc-4d-nyerere": "/packages images/nyerere-elephants.jpg",
  "sc-5d-nyerere": "/packages images/nyerere-elephants.jpg",
  "sc-2d-ruaha": "/packages images/ruaha.jpg",
  "sc-3d-ruaha": "/packages images/ruaha.jpg",
  "sc-4d-ruaha": "/packages images/ruaha.jpg",
  "sc-5d-ruaha": "/packages images/ruaha.jpg",
  "sc-4d-nyerere-ruaha": "/selous.jpg",
  "sc-6d-nyerere-ruaha": "/selous.jpg",
  "sc-7d-nyerere-ruaha-mikumi": "/selous.jpg",
  "sc-14d-southern-explorer": "/selous.jpg",
};

export async function generateStaticParams() {
  return southernCircuitPackages.map((pkg) => ({ id: pkg.id }));
}
...
export async function generateMetadata({
  ...
  const pkg = southernCircuitPackages.find((p) => p.id === id);
  if (!pkg) return {};
  return {
    title: `${pkg.title} — ${pkg.subtitle}`,
    description: `Explore the ${pkg.title} safari package (${pkg.subtitle}). ${pkg.days} days visiting ${pkg.parks.join(", ")} in Tanzania's Southern Circuit.`,
  };
}

export default async function SouthernPackageDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  const waMessage = encodeURIComponent(
    `Hi Ikamba Adventure! I'm interested in the "${pkg.title}" (${pkg.subtitle}) Southern Circuit safari. Please share details and pricing.`
  );
    ...
    <>
      ...
      <section className="relative h-[65vh] min-h-[440px] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
          ...
          <div className="flex items-center gap-2 text-white/60 text-sm font-heading mb-5">
            ...
            <Link href="/safaris/southern-circuit" className="hover:text-white transition-colors">Southern Circuit</Link>
            <ChevronRight size={14} />
      ...
      <section className="py-14 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-heading font-black text-2xl text-gray-900 mb-8">More Southern Circuit Packages</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {southernCircuitPackages
              .filter((p) => p.id !== pkg.id)
              .slice(0, 3)
              .map((related) => (
                <Link
                  key={related.id}
                  href={`/safaris/southern-circuit/${related.id}`}
